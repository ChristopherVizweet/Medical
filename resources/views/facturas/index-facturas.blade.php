            ...
            <table class="w-full text-left bg-white dark:text-gray-200 dark:bg-gray-500">
                <thead class="bg-gray-200 dark:text-gray-200 dark:bg-gray-600">
                    <tr>
                        ...
                        <th class="px-6 py-2">Folio Fiscal</th>
                        <th class="px-6 py-2">Fecha de Emisión</th>
                        <th class="px-6 py-2">Fecha de Vencimiento</th>
                        <th class="px-6 py-2">Quién recibe</th>
                ...
                <tbody>
                    @forelse ($facturas as $factura)
                         <tr class="">
                            ...
                            <td class="px-6 py-2">{{$factura->nombre_emisor ?? "SIN RFC"}}</td>
                            <td class="px-6 py-2">{{$factura->nombre_receptor ?? "Sin nombre de emisor"}}</td>
                            <td class="px-6 py-2">{{$factura->folio_factura ?? "Sin folio"}}</td>
                            <td class="px-6 py-2">{{$factura->folio_fiscal ?? "Sin folio fiscal"}} </td>
                            <td class="px-6 py-2">{{$factura->fecha_emision ?? "Sin fecha de emisión"}}</td>
                            <td class="px-6 py-2">{{$factura->fecha_vencimiento ?? "Sin fecha de vencimiento"}}</td>
                            <td class="px-6 py-2">{{$factura->recibe_factura ?? "No recibio almacén"}}</td>
