...
<x-mode-button id="theme-toggle" class="float-right" >
   ...
   <script>
    ...
    <div class="container mx-auto mt-4">
            ...
            <h1 class="text-2xl font-bold mb-4 text-black dark:text-white">Lista de Servicios</h1>
                ...
                <tbody>
                    @forelse ($services as $service)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-2">{{ $service->nameInstalation}}</td>
                            <td class="px-4 py-2">
                                <a href="{{route('edit-service', $service->id) }}" class="text-blue-600 hover:underline">Editar </a>| 
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-2 text-center">{{('Servicios no encontrados') }}</td>
                        </tr>
                    @endforelse
                </tbody>
                <x-primary-button class="mt-4">
                    <a href="{{ route('create-service') }}" class="text-dark"> 
                         {{ __('Nuevo servicio') }}
                     </a> 
                </x-primary-button>
            </table>
