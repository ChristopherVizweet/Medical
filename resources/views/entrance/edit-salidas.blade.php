<x-guest-layout>
    <form method="POST" action="{{ route('edit-salidas', $salidas->id) }}">
        @csrf
        @method('PUT')
        ...
        <div>
         ...
         <div>
            ...
            <select class="mt-1 block w-full" readonly="true" name="encargado_almacen" id="encargado_almacen" required>
                <option value="">-Seleccionar-</option> 
                @foreach($empleados as $empleado)
                    <option value="{{ $empleado->id }}" {{ $salidas->productos->first()->encargado_almacen == $empleado->id ? 'selected' : '' }}>
                        {{ $empleado->Nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select class="mt-1 block w-full" readonly="true" name="encargado_envio" id="encargado_envio" required>
                <option value="">-Seleccionar-</option> 
                @foreach($empleados as $empleado)
                <option value="{{ $empleado->id }}" {{ $salidas->productos->first()->encargado_envio == $empleado->id ? 'selected' : '' }}>
                    {{ $empleado->Nombre }}
                </option>
            @endforeach
            </select>
        ...
        <div>
            ...
            <select class="mt-1 block w-full" readonly="true" name="encargado_recibe" id="encargado_recibe" required>
                <option value="">-Seleccionar-</option> 
                @foreach($empleados as $empleado)
                <option value="{{ $empleado->id }}"{{ $salidas->productos->first()->encargado_recibe == $empleado->id ? 'selected' : '' }}>
                    {{$empleado->Nombre}}
                </option>
            @endforeach
            </select>
        ...
        <div>
             <x-input-label for="observaciones_movimiento" :value="__('Obervaciones')" />
            <x-text-input  id="observaciones_movimiento" class="block mt-1 w-full" type="text" name="observaciones_movimiento" value="{{ $salidas->observaciones_movimiento }}" required />
        </div>
<div id="productos-wrapper" class="space-y-4 border p-4 rounded-lg mt-3">
    @foreach($salidas->productos as $i => $prod)
        <div class="producto-row grid grid-cols-5 gap-5 items-center">
            ...
            <!-- Cantidad recibida -->
            <div>
                <x-input-label for="productos[{{ $i }}][cantidadA]" :value="__('Cantidad recibida')" />
