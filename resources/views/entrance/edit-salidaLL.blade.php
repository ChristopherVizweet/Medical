        ...
        <div class="mt-4">
            ...
            <div class="producto-row grid grid-cols-4 gap-4 items-center">
                ...
                <div class="hidden">
                    ...
                    <x-text-input id="tipoMovimiento" class="mt-1 block w-full" type="text" name="tipoMovimiento"
                        value="{{ old('tipoMovimiento', $salidas->tipoMovimiento) }}" required />
                    <x-input-error :messages="$errors->get('tipoMovimiento')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="obra_movimiento" :value="__('Obra')" />
                    <x-text-input readonly="true" id="obra_movimiento" class="mt-1 block w-full" type="text"
                        name="obra_movimiento" value="{{ $salidas->productos->first()->obra_movimiento }}" required />
                    <x-input-error :messages="$errors->get('obra_movimiento')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="fecha_movimiento" :value="__('Fecha')" />
                    <x-text-input readonly="true" id="fecha_movimiento" class="mt-1 block w-full" type="date"
                        name="fecha_movimiento" value="{{ $salidas->fecha_movimiento }}" required />
                    ...
                    <select class="mt-1 block w-full" readonly="true" name="empleado_id" id="empleado_id" required>
                        ...
                        @foreach ($empleados as $empleado)
                            <option value="{{ $empleado->id }}"
                                {{ $empleado->id_empleado == $salidas->Nombre ? 'selected' : '' }}>
            ...
            <x-text-input id="observaciones_movimiento" class="block mt-1 w-full" type="text"
                name="observaciones_movimiento" value="{{ $salidas->observaciones_movimiento }}" />
        </div>
            ...
            @foreach ($salidas->productos as $i => $prod)
                <div class="producto-row grid grid-cols-5 gap-5 items-center">
                    ...
                    <div>
                        ...
                        <select readonly="true" name="productos[{{ $i }}][product_id]"
                            ...
                            @foreach ($productos as $producto)
                                <option readonly="true" value="{{ $producto->id }}"
                                    {{ $producto->id == $prod->product_id ? 'selected' : '' }}>
                                    {{ $producto->first()->name_product }} Diametro
                                    {{ $producto->diameterMM_product }}mm</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input readonly="true" id="productos[{{ $i }}][cantidadR]"
                            class="mt-1 block w-full" type="number" name="productos[{{ $i }}][cantidadR]"
                            value="{{ old('productos.' . $i . '.cantidadR', $prod->cantidadR) }}" required />
                    ...
                    <div>
                        ...
                        <x-text-input readonly="true" id="productos[{{ $i }}][cantidadA]"
                            class="mt-1 block w-full" type="number" name="productos[{{ $i }}][cantidadA]"
                            value="{{ old('productos.' . $i . '.cantidadA', $prod->cantidadA) }}" required />
