...
<head>
    <meta charset="UTF-8">
    <title>Gastos de Verificaciones del Vehículo</title>
    <style>
...
<body>
    <div class="header">
        <h1>Resumen de gastos de verificaciones</h1>
        ...
        <tbody>
            @forelse ($verificaciones as $verificacion)
                <tr>
                    ...
                    <td>{{ $verificacion->fecha_pago_verificacion ?? 'Sin información' }}</td>
                    <td>{{ $verificacion->fecha_proxima_verificacion ?? 'Sin información' }}</td>
                    <td>${{ number_format((float) ($verificacion->monto_verificacion ?? 0), 2) }}</td>
                    <td>{{ $verificacion->observaciones_verificacion ?? 'Sin información' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5">No hay registros de verificaciones para este vehículo.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
