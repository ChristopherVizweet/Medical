...
<head>
    <meta charset="UTF-8">
    <title>Gastos del Seguro del Vehículo</title>
    <style>
...
<body>
    <div class="header">
        <h1>Resumen de gastos del seguro</h1>
    ...
    <table>
        ...
        <tbody>
            @forelse ($seguros as $seguro)
                <tr>
                    <td>{{ $seguro->id }}</td>
                    <td>{{ $seguro->empresa_seguro ?? 'Sin información' }}</td>
                    <td>{{ $seguro->fecha_pago_seguro ?? 'Sin información' }}</td>
                    <td>{{ $seguro->fecha_proxima_seguro ?? 'Sin información' }}</td>
                    <td>{{ $seguro->fecha_expirar_seguro ?? 'Sin información' }}</td>
                    <td>${{ number_format((float) ($seguro->monto ?? 0), 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="6">No hay registros de seguros para este vehículo.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
